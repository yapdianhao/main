...
and back-end. It involves the `Ui`, `Logic`, `Model`, and `Storage`.
...
therefore they are closely relatable to each other. To successfully design such a class that connects a wide
range of components, a complete understanding and thorough analysis is required.
*** Moreover, this feature incorporates a Ui pane that shows all the `Reminder` that are in the queue for users' convenience.
Users are able to view the reminders similar to that of the tasks, and take action accordingly.
...
* *Minor enhancement*: Increased code coverage through writing new tests and improved existing tests.
...
**** Requesting usage for third-party libraries (controlsfx) https://github.com/nus-cs2103-AY1920S2/forum/issues/81[#81]
