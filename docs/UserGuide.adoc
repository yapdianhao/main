...
Morning Call Displays the tasks that are due today, and Night Debrief displays the tasks that have been completed within the day.

The tasks listed in Morning Call and Night Debrief are displayed in under the Summary tab as separate sections under "Due Today" and "Completed Today" respectively.
...
or its shortcut `:s` to manually switch to the summary tab.
...
[.text-center]
.Expected result after running `done 5`, which marks the task for ACC1101 Tutorial 4 as completed
...
or its shortcut `:t` to manually switch to the task list tab.
...
[TIP]
Tasks which are due and completed will not be shown, as we assume you no longer need them.
...
or its shortcut `:c` to manually switch to the calendar tab.
...
or its shortcut `:p` to manually switch to the producivity tab.
...
image::ug_figures/start.png[width="600"]
...
image::ug_figures/stop.png[width="600"]
...
Whenever you boot JelphaBot, if there is any tasks that will be reminded, JelphaBot will show
a list of tasks that will expire soon, and tasks that have expired but not done.
