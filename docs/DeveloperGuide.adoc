...
| _italics_ | A command which can be called in JelphaBot. +
...
image::SummaryPanelClassDiagram.png[width=500]
...
The _delete command calls `Model#commitJelphaBot()`, causing the modified state of JelphaBot after the `delete 5` command executes to be saved in the `jelphaBotStateList`, and the `currentStatePointer` is shifted to the newly inserted JelphaBot state.
