...
To view the respective tasks, the user enters the `summary` command to switch to the summary tab panel.

[[Summary-SwitchTab-Sequence-Diagram]]
.Sequence diagram of execution of the `summary` command
image::SummaryCommand.png[]
...
===== Aspect 2: Method of storing `ObservableList<Task>` of tasks for each day card (Implementation of Dot Indicator)
...
// tag::undoredo[]
=== Undo/Redo feature [Proposed to v2.0]
...
. Input a date belonging to the calendar month to view tasks due
...
. Input a date not belonging to the calendar month to view tasks due
.. Prerequisites: Navigate to the calendar with the `calendar` command (or other variants as listed above).
