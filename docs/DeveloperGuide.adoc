...
*Reason for chosen implementation:* +
No need to re-render all 3 parts when changes are made to only one part.
...
*Reason for chosen implementation:* +
Adding and deleting tasks does not affect the task being timed.
...
*Reason for chosen implementation:* +
Paragraphing increases readability and the progress bar provides visual aid.
// end::productivity[]
...
The `Reminder` feature was implemented by introducing a new set of classes to `Model`. A new `Reminder` class is stored in
