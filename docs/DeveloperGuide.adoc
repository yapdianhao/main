...
As such, we have decided to implement a tab system for ROR to organise the available commands into their respective features. JelphaBot provides 4 different tabs for the users,
...
The tabs of the application are defined using a [.java]#Tab# enum and the current mode of the application is stored as a private attribute [.java]#mode# in [.java]#LogicManager#. Users can switch between modes in JelphaBot using the lower case names of each tab as commands. For instance, `calendar. When the tab of the application is changed, we need to update the:

* [.java]#Logic# component so that the [.java]#tab# attribute in [.java]#LogicManager# reflects the new current tab, since this is used to check if a command can be executed,
...
For all these commands, these updates are done using a new [.java]#toTab# attribute added to the [.java]#CommandResult# object. As an example, the figure below shows the sequence diagram for when a user executes the `:p` command:

.Component interactions for `:p` command
image::productivityTabSequenceDiagram.png[width="800"]

Upon execution of the `:p` command, [.java]#ProductivityCommand#generateCommandResult()# will generate a [.java]#CommandResult# whose [.java]#toTab# attribute is set to [.java]#PRODUCTIVITY# and return it to the [.java]#LogicManager#. Now, the updates can be done for the respective components:
...
* *Current Solution*: UI displays problems from a `FilteredList<SubgroupTaskList>` and use `setPredicate` to hide empty categories without removing them.
listeners n sets alternative: uniquemodulecodetasklist
