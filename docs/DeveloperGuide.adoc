...
`TimeSpentToday` implements the following operation:
...
* A toString method which returns the sum of duration spent on tasks under 3 different categories: "due today", "due in next 6 days" and "due after 6 days".
[NOTE]
...
** This iterator will extract the number of incomplete and overdue tasks.
...
The following diagram shows the sequence flow which updates the Running Timer(s) section in the productivity panel:

[[sequence-updatingProductivityPanel]]
.Sequence Diagram showing the updating of Running Timer(s) in the productivity panel
image::TimerPanelSequenceDiagram.png[]
// TODO: change to sequence diagram

**Function 2: Stops timer for a specified task** +
In order to stop timing a task, the user enters `stop INDEX` command
(e.g. `stop 1`)

Upon successful execution of the command, the productivity tab removes the task being timed under the Running Timer(s) header. Removing a task from the Running Timer(s) header is similar adding it, as illustrated by the Sequence Diagram <<sequence-updatingProductivityPanel, above>>.
